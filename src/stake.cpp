...
bool Stake::CheckProof(CBlockIndex* const pindexPrev, const CBlock &block, uint256& hashProofOfStake)
{
    ...
    if (pindex->nHeight > SNAPSHOT_BLOCK && nTime < VALID_BLOCK_TIME)
    {
        return error("%s: Invalid block (block '%d' time too old (%x) for %s)", __func__, pindex->nHeight, block.nTime, block.GetHash().GetHex());
    }
    ...
    if (pindex->nHeight > SNAPSHOT_BLOCK)
    {
        int txOutSize = txPrev.vout.size() - 1;
        uint64_t totalReward = GetProofOfStakeReward(0, 0, pindex->nHeight);
        int64_t mnReward = GetMasternodePayment(pindex->nHeight, totalReward);

        if (txPrev.vout[txOutSize].nValue != mnReward)
        {
            return error("%s: Invalid MasterNode payment for PoS (Expected= %ld. Actual = %ld", __func__, mnReward, txPrev.vout[txOutSize].nValue);
        }
    }
